					...
					<div class="py-3 window-question bg-white">
						<div id="first-section" class="text-center">
							...
							<div>
								...
								<div class="mt-3 text-muted">당신은 혹시<%=request.getContextPath()%> ISFP?</div>
							</div>
						</div>
						<div id="second-section" class="mt-5 border-top-black">
							<div class="text-left">
								...
								<div class="m-3 me-4 fs-6 font-ex">
									<ul>
										<li>프로집순이. 집에만 들어가면 연락두절된다. “지금 뭐해?” “누워있지”. 침대에 누워서 코딩하기? 쌉가능. </li>
										<li>이것좀 해 주실 수 있나요? ‘아니오’ 라는 말을 못해 집에 와서 야근 하며 후회하는 스타일. 부탁받으면 거절을 잘 못하지만 또 내가 부탁을 잘 하지도 못한다. </li>
										<li>프로젝트 마감이 내일이라구요<%=request.getContextPath()%>? 좀만 이따 시작할게요,, 발등에 불이 떨어져서 타들어가도록 미룰 수 있을때까지 일을 끝까지 미룬다. 그런데 어떻게든 다 해내고 심지어 잘해와서 주변 사람들이 깜짝 놀람.</li>
										<li>스트레스 너무 많이 주지 마세요 ㅠㅠ 갑자기 학교나 직장을 때려칠 수도 있다구요.</li>
										<li>내 발자국은 어디에도 남기기 싫어<%=request.getContextPath()%> 카톡 프사는 텅텅, 인스타는 눈팅용. 나에 대해 세상에 알리기에는 만사가 다 귀찮다.️</li>
									</ul>
								</div>
							</div>
						</div>
						<div id="third-section" class="mt-5 border-top-black">
							<div class="text-center">
								...
								<div class="m-3 fs-6 font-ex">
									<p class="text-muted">당신은 세심하고 주의력이 필요한 일에 큰 재능을 보이네요. 또한 실질적인 도움을 제공해서 재빨리 문제를 해결 할 수 있는 일에 흥미가 있어요.</p>
								</div>
								<div id="job" class="text-center">
									<button class="btn btn-light btn-job fs-5">빅데이터 분석가</button>
								</div>
								<div class="m-3 fs-6 font-ex">
									<p>빅데이터 분석가는 수많은 데이터 속에서 트렌드를 읽어내 부가가치가 높은 결과물을 도출해내는 일을 담당해요. 대량의 빅데이터를 관리하고 분석해 사람들의 행동패턴이나 시장 경제상황 등을 예측하기도 합니다.</p>
								</div>
							</div>
						</div>
								...
								<div class="col pe-0">
									<div id="type-good" class="text-center">
										<button class="btn btn-light btn-type ">
											...
											<h3>꼰대 개발자</h3>
											<img src="<%=request.getContextPath()%>/resources/img/teamA/ESTJ.png"
												class="img-type mx-auto d-block" alt="<%=request.getContextPath()%>.">
										</button>
									</div>
								</div>
